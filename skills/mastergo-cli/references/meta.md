...
4. Use the `targetLayerId` from the `action` field to call `mastergo dsl --file-id <fileId> --layer-id <targetLayerId>` to get data.
5. Analyze the page data to check if there is an `interactive` field. If it exists, this field contains information about the current node's navigation to another page. You must continue to call `mastergo dsl` according to the `interactive` field.
...
Use `mastergo dsl --file-id <fileId> --layer-id 0:1` to parse the 0:1 page and analyze the data.
...
Repeat this process until the data parsed by `mastergo dsl` no longer contains the `interactive` field.
Then, use `mastergo dsl` to parse the 0:2 page and repeat the steps for the 0:1 page.

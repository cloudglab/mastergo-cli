...
4. For each `targetLayerId` from the `action` field, use the section-by-section workflow to get page data:
   - **Step 4a**: Call `mastergo design-sections --file-id <fileId> --layer-id <targetLayerId>` WITHOUT `--section-index` to get the list of all sections.
   - **Step 4b**: For each section (index 0 to N-1), call `mastergo design-sections --file-id <fileId> --layer-id <targetLayerId> --section-index <i>` to get the full DSL. Fetch in BATCHES of 3-5 at a time — do NOT request all sections simultaneously to avoid timeouts.
...
Use `mastergo design-sections --file-id <fileId> --layer-id 0:1` to get the section list for page 0:1 (without `--section-index`), then fetch each section's DSL by calling with `--section-index 0`, `--section-index 1`, ... until all sections are fetched. Fetch in batches of 3-5 at a time to avoid timeouts.
